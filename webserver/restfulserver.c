...
void get_addressbook(int fd, char *body) {

	char tmp_json[MAX_RESPONSE_SIZE] = { 0 };
	char line[MAX_RESPONSE_SIZE] = { 0 };

	char *parser1 = strtok(body, "\"");
	char *parser2 = strtok(NULL, "\"");
	char *parser3 = strtok(NULL, "\"");
	...
	while(fgets(line, MAX_RESPONSE_SIZE, fp) != NULL) {
		if (strstr(line, tmp_name) != NULL) {
			  break;
		 }
		 memset(line, 0, MAX_RESPONSE_SIZE);
	}

	fclose(fp);

	sprintf(tmp_json, "{ %s }", line);
	printf("Response : %s\n", tmp_json); 
  	send_response(fd, "HTTP/1.1 200 OK", "application/json", tmp_json, strlen(tmp_json));
}
...
void handle_http_request(int fd) {
  ...
  if (strcmp(request_type, "GET") == 0) {
    ...
    if (strcmp(request_path, "/save") == 0) {
      post_save(fd, body);
    } else if (strcmp(request_path, "/address") == 0) {
    	get_addressbook(fd, body);
    } else {
      resp_404(fd);
    }
  } else {
